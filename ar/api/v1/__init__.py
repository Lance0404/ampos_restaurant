...
from ar.api.errors import QuantityBelowZero, RequiredKeyMissing
...
def error_500(e):
    # current_app.logger.error(f'this is a 404 error')
    response = dict(status=0, msg="500 Error from ar")
    return jsonify(response), 500

# @bp.errorhandler(QuantityBelowZero)
# def handle_QuantityBelowZero_error(e):
...
@bp.errorhandler(RequiredKeyMissing)
def handle_RequiredKeyMissing_error(e):
    current_app.logger.error(f'this is a RequiredKeyMissing Error')
    response = dict(status=0, msg="RequiredKeyMissing from server")
